...
/**
 * Puppeteer binding for a11y-core -- scans a real, already-rendered page.
 *
 ...
 * doesn't need any a11y-core engine support: Puppeteer drives every frame
...
class A11yCoreBuilder {
   ...
   * Puppeteer page.evaluate() argument crosses a JSON boundary that cannot
   ...
   * Puppeteer `ElementHandle` (attached as `occurrence.elementHandle`), so
   * callers can `.click()`/`.screenshot()` the flagged element directly
   ...
   * opt-in. Uses `<page-or-frame>.$()` rather than `evaluateHandle` since a
   * plain `ElementHandle` is all callers need. Combines with `.frames(true)`:
   * each frame's occurrences are resolved against that frame's own document,
   * not the top page's.
   */
  ...
  async analyze() {
    ...
    // see a11y-core-playwright's own A11yCoreBuilder.js for the full
    ...
    // through with no wrapper/eval() trick needed -- see a11y-core's own

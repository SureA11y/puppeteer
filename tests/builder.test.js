...
test('A11yCoreBuilder: frames(true) scans a genuinely cross-origin iframe (no a11y-labs engine support needed for this -- see ../ROADMAP.md §2c)', async () => {
  ...
  try {
    ...
    // frame scanning needs no engine work, the same claim the Playwright
    // binding already verified for itself (Puppeteer also drives the browser
    // via CDP at the automation-process level, so the same reasoning
    // transfers -- confirmed here with a real run, not assumed).

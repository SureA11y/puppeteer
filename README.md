...
Also read [`../a11y-core/docs/BINDING_AUTHORS_GUIDE.md`](../a11y-core/docs/BINDING_AUTHORS_GUIDE.md) before starting — a reference written specifically for a *new* binding like this one, distinguishing what's already engine-level (works through a generic `.options()`/`runOnly` passthrough with zero binding code, e.g. WCAG-version tag filtering, `structuralPath`) from what every binding has to build itself (element refs, `reportOnly`-style verbosity filtering, and — directly relevant here, since Puppeteer's `page.evaluate()` crosses the same kind of realm boundary Playwright's does — the serialization-boundary caveat for `engineOptions.customRules`' `runInPage`/`applicability`, which must reach the page as a function-source string, not a live function). It cites `a11y-core-playwright`'s `.elementRef()`, `.reportOnly()`, `.frames(true)`, and `.withCustomRules()` by name as the worked examples this project's own builder should mirror.
